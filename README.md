...
- `bmad-sprint-planning` — the skill ships `sprint_plan.py` in its `scripts/` folder. Orchestrator's Phase 4 invokes that script directly via `uv run python .../bmad-sprint-planning/scripts/sprint_plan.py generate --set <key>=done` to mark converged stories as done in `sprint-status.yaml`. (The script uses PEP 723 inline metadata for its `ruamel.yaml` dependency — `uv run python` resolves it automatically, matching the pattern used by this module's own `write-state.sh`.) The story-level done transition works. Epic-level advancement and spec→ready-for-dev upgrade inside `sprint_plan.py` have a known filename mismatch with this module's producer format — see `Known limitations`.
